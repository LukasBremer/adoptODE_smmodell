...
Parameters of the EOM are distinguished between three categories:
  1. params:    The normal general parameters of the system which are
                unknown.
...
following:
    ...
    # For fitting Data (for details see dataset_adoptODE):
    ...
    # For simulating Data (for details see simple_simulation):
...
def define_system_dummy(**kwargs_sys):
    """Example of a very simple system following the equation dy/dt = a*y + b
    ...
    Returns:
      A number of functions specific to the system in question:
      eom:    The system's EOM
      loss: The loss function
      gen_params: The three different types of parameters
      gen_y0: The initial conditions (For a single realization!)
      {}: A dictionary of internal functions, for example building up a
    ...
    def gen_params() -> Tuple[PyTree, PyTree, PyTree]:
        ...
        Returns:
          Three PyTrees, the first beeing the params, the second iparams
        ...
        return {
            ...
            'b': np.random.rand(kwargs_sys['N_sys'])
        }, {}
    ...
    def gen_y0():
        ...
        Returns:
          A PyTree saving the state of a single system at a single time.
        """
        return {'State': np.random.rand()}

    @jax.jit
    def eom(y: PyTree, _t: float, params: PyTree, iparams: PyTree,
            _exparams: PyTree) -> PyTree:
    ...
    @jax.jit
    def loss(ys: PyTree, _params: PyTree, _iparams: PyTree, _exparams: PyTree,
             ys_targets: PyTree) -> float:
        ...
        """
        return jnp.sum((flatten_util.ravel_pytree(ys)[0] -
                        flatten_util.ravel_pytree(ys_targets)[0])**2)
...
class EquationsNODE():
    ...
    adoptODEs is given in Chen, Ricky TQ, et al. "Neural ordinary differential
    ...
    following meaning:
      ...
      y:        current state of a single system.
      params:   Set of parameters of a single system. (See define_system_dummy)
      ...
      ys:       The state of a system at all times in t_eval, the timepoints
                being the leading axis.
      ys_targets: Same as ys, but not results of the current model, but the
                provided data to be fitted.

    Args:
      eom: EOM as given in define_system_dummy above.
      loss: loss function as given in define_system_dummy above
      kwargs_adoptODE: Hyperparameters for adoptODE, for details see dataset_adoptODE

    Returns:v_forward, gradient_adjoint_batch, back_check, grad_check, v_loss
      v_forward:
        Vectorized forward function for a batch of systems
      gradient_adjoint_batch:
        The main routine, yielding the gradients of a batch of systems with
        given target values.
      backwards_check:
        A test function to check if the backwards pass is stable in terms of the
        occuring ODE solutions of the system.
      gradient_check:
        A test function to check if the backwards pass is stable in terms of the
        occuring ODE solutions to the gradients and additional dimensions.
      v_loss:
        A vectorized version of loss, computing the loss for a batch of systems.
    """
    ...
    def __init__(self, eom, loss, **kwargs_adoptODE):
        ...
        def r_forward(y0, t_evals, params, iparams, exparams, ys_target,
                      t_stop_idx, t_reset_idcs):
            ...
            ys = tree_util.tree_map(
                lambda x: jnp.nan * jnp.zeros_like(x), ys_target)
            ...
            max_slice_size = max(
                [b - a for a, b in zip(t_start_idcs, t_end_idcs)])

            def r_forward_body(i, state):
                ys, params, iparams, exparams, y0 = state

                t_start, t_end = jnp.array(t_start_idcs)[
                    i], jnp.array(t_end_idcs)[i]
                ...
                t_evals_i = jnp.where(
                    ...
                    jax.lax.dynamic_slice(t_evals_ext, [t_start],
                                          [max_slice_size + 1]), t_evals[t_end])
                ys_new = odeint(eom,
                                y0_i,
                                t_evals_i,
                                params,
                ...
                def update_a_time(idx, update_state):
                    ...
                    ys = tree_util.tree_map(
                        lambda x, y: x.at[t_start + idx].set(y[idx]), ys,
                        ys_new)
                    return (ys, ys_new, t_start)

                ys = jax.lax.fori_loop(1, t_end - t_start + 1, update_a_time,
                                       update_a_time_state)[0]
        ...
        def v_forward(y0_batch, t_evals, params, iparams_batch, exparams_batch):
            ...
            return jax.vmap(forward, in_axes=(0, t_evals_vec_axis, None, 0,
                                              0))(y0_batch, t_evals, params,
                                                  iparams_batch, exparams_batch)

        self._v_forward = v_forward

        @partial(jax.jit, static_argnums=(6, 7))
        def v_r_forward(y0_batch, t_evals, params, iparams_batch, exparams_batch,
                        ys_target_batch, t_stop_idx, t_reset_idcs):
            """Vectorized r_forward function, dealing with a batch of systems."""
            return jax.vmap(r_forward,
                            in_axes=(0, None, None, 0, 0, 0, None,
                                     None))(y0_batch, t_evals, params, iparams_batch,
                                            exparams_batch, ys_target_batch,
                                            t_stop_idx, t_reset_idcs)
        ...
        def dloss_dp(ys, params, iparams, exparams, ys_target):
            ...
            return jax.grad(lambda params: loss(ys, params, iparams, exparams,
                                                ys_target))(params)
        ...
        def dloss_dip(ys, params, iparams, exparams, ys_target):
            ...
            return jax.grad(lambda iparams: loss(ys, params, iparams, exparams,
                                                 ys_target))(iparams)
        ...
        if kwargs_adoptODE['multi_measurement_constraint'] is None:
            ...
            def multi_meas_grad(ys, params, iparams, exparams, ys_target):
                return jax.grad(
                    ...
                    (ys, params, iparams, exparams, ys_target))(iparams)

        self.multi_meas_grad = multi_meas_grad

        @jax.jit
        def v_loss(ys_batch, params, iparams_batch, exparams_batch,
                   ys_target_batch):
            """Vectorized loss function handling a batch of systems."""
            return jax.vmap(loss, in_axes=(0, None, 0, 0,
                                           0))(ys_batch, params, iparams_batch,
                                               exparams_batch, ys_target_batch)
        ...
        if kwargs_adoptODE['EOM_dt_dependent'] is True:
            ...
            def aug_dyn(aug_dyn_state, t_fake, dt, params, iparams, t2, exparams):
                ...
                y = aug_dyn_state[0]
                a = aug_dyn_state[1]
                Jy_vjp = jax.vjp(lambda y: eom(y, t, dt, params, iparams, exparams),
                                 y)[1]
                Jp_vjp = jax.vjp(
                    lambda params: eom(y, t, dt, params, iparams, exparams), params)[1]
                Jip_vjp = jax.vjp(
                    lambda iparams: eom(y, t, dt, params, iparams, exparams),
                    iparams)[1]
                Jy = Jy_vjp(a)[0]
                Jp = Jp_vjp(a)[0]
                Jip = Jip_vjp(a)[0]
                return (tree_util.tree_map(lambda x: -x,
                                           eom(y, t, dt, params, iparams,
                                               exparams)), Jy, Jp, Jip)
        else:
            ...
            def aug_dyn(aug_dyn_state, t_fake, params, iparams, t2, exparams):
                ...
                y = aug_dyn_state[0]
                a = aug_dyn_state[1]
                Jy_vjp = jax.vjp(lambda y: eom(
                    y, t, params, iparams, exparams), y)[1]
                Jp_vjp = jax.vjp(lambda params: eom(y, t, params, iparams, exparams),
                                 params)[1]
                Jip_vjp = jax.vjp(lambda iparams: eom(y, t, params, iparams, exparams),
                                  iparams)[1]
                Jy = Jy_vjp(a)[0]
                Jp = Jp_vjp(a)[0]
                Jip = Jip_vjp(a)[0]
                return (tree_util.tree_map(lambda x: -x,
                                           eom(y, t, params, iparams,
                                               exparams)), Jy, Jp, Jip)
        ...
        def backup_body(i, state):
            ...
            """
            aug_dyn_state, length_backup, params, iparams, t_backups, exparams, backups, time_decay = state
            n_backups = kwargs_adoptODE['N_backups']
            sol = odeint_back(aug_dyn,
                              aug_dyn_state,
                              ...
                              rtol=kwargs_adoptODE['rtol'],
                              atol=kwargs_adoptODE['atol'])
            aug_dyn_state = (tree_util.tree_map(
                lambda x: x[n_backups - i - 1], backups), *(tree_util.tree_map(
                    lambda x: x[-1] * time_decay**length_backup, sol[1:])))
            return (aug_dyn_state, length_backup, params, iparams, t_backups,
                    exparams, backups, time_decay)

        self._backup_body = backup_body

        @jax.jit
        def backup_loop(y_t2, y_t1, a_t2, params, iparams, exparams, ts, dLdp,
                        dLdip, time_decay):
            ...
            if n_backups > 1:
                ...
                backups = tree_util.tree_map(lambda x, y: jnp.stack((x, y)), y_t1,
                                             y_t2)
            aug_dyn_state = (y_t2, a_t2, dLdp, dLdip)
            state_init = (aug_dyn_state, length_backup, params, iparams, t_backups,
                          exparams, backups, time_decay)
        ...
        def backwards_body(i, state):
            ...
            Returns: The state after passing backwards one interval between to times
              in t_evals.
            """
            dLdp, dLdip, a_old, ys, params, iparams, exparams, t_evals, ys_target, t2_index, time_decay = state
            index = t2_index - i
            ...
            a_t2 = tree_util.tree_map(
                ...
                dloss_dy(ys, index, params, iparams, exparams, ys_target), a_old)
            sol = backup_loop(y_t2, y_t1, a_t2, params, iparams, exparams,
                              t_evals[jnp.array(
                                  [index, index - 1])], dLdp, dLdip,
                              time_decay)
            dLdp = sol[2]
            dLdip = sol[3]
            a_old = sol[1]
            return (dLdp, dLdip, a_old, ys, params, iparams, exparams, t_evals,
                    ys_target, t2_index, time_decay)

        self._backwards_body = backwards_body

        @jax.jit
        def backwards_loop(ys, t_eval, params, iparams, exparams, ys_target,
                           time_decay, t_target_range):
            ...
            """
            dLdp, dLdip = tree_util.tree_map(jnp.zeros_like,
                                             params), tree_util.tree_map(
                                                 jnp.zeros_like, iparams)
            a_old = tree_util.tree_map(lambda x: jnp.zeros(x.shape[1:]), ys)
            state_init = (dLdp, dLdip, a_old, ys, params, iparams, exparams, t_eval,
                          ys_target, t_target_range[1], time_decay)
            state_final = lax.fori_loop(0, t_target_range[1] - t_target_range[0],
                                        backwards_body, state_init)
            return state_final[0], state_final[1], state_final[2]

        self._backwards_loop = backwards_loop

        @partial(jax.jit, static_argnums=(6, 8))
        def gradient_adjoint(y0,
                             t_evals,
                             params,
                             iparams,
                             exparams,
            ...
            if t_stop_idx is None and t_reset_idcs is None:
                ...
                ys = r_forward(y0, t_evals, params, iparams, exparams, ys_target,
                               t_stop_idx, t_reset_idcs)
                y0_to_take = jnp.arange(len(t_reset_idcs))
            # Vectorization for different reset initial times
            dLdp, dLdip, dLdy0 = jax.vmap(backwards_loop,
                                          in_axes=(None, None, None, None, None,
                                                   None, None, 0),
                                          out_axes=(0, 0, 0))(ys, t_evals, params,
                                                              iparams, exparams,
                                                              ys_target, time_decay,
                                                              t_idx_range)

            dLdp = tree_util.tree_map(
                lambda x, y: jnp.sum(x, axis=0) + jnp.nan_to_num(y), dLdp,
                dloss_dp(ys, params, iparams, exparams, ys_target))
            dLdip = tree_util.tree_map(
                lambda x, y: jnp.sum(x, axis=0) + jnp.nan_to_num(y), dLdip,
                dloss_dip(ys, params, iparams, exparams, ys_target))
            dLdy0 = tree_util.tree_map(
                lambda x, y: x[y0_to_take] + jnp.nan_to_num(y), dLdy0,
        ...
        @partial(jax.jit, static_argnums=(6, 8))
        def gradient_adjoint_batch(y0_batch,
                                   t_evals,
                                   params,
                                   iparams_batch,
                                   exparams_batch,
            ...
            dLdp, dLdip, dLdy0, ys_batch = jax.vmap(
                gradient_adjoint,
                in_axes=(0, t_evals_vec_axis, None, 0, 0, 0, t_evals_vec_axis,
                         t_evals_vec_axis, None),
                out_axes=(0, 0, 0, 0))(y0_batch, t_evals, params, iparams_batch,
                                       exparams_batch, ys_target_batch, t_stop_idx,
                                       time_decay, t_reset_idcs)  # jit removed
            dLdp = tree_util.tree_map(lambda x: jnp.mean(x, axis=0), dLdp)
            dLdip = tree_util.tree_map(
                lambda x, y: x + y, dLdip,
                multi_meas_grad(ys_batch, params, iparams_batch, exparams_batch,
                                ys_target_batch))
    ...
    def forward(self, y0_batch, t_evals, params, iparams_batch, exparams_batch):
        return self._v_forward(y0_batch, t_evals, params, iparams_batch,
                               exparams_batch)

    def resetted_forward(self, y0_batch, t_evals, params, iparams_batch,
                         exparams_batch, ys_target_batch, t_stop_idx,
                         t_reset_idcs):
        t_r_i_here = (0, ) if t_reset_idcs is None else t_reset_idcs
        t_s_i_here = t_evals.shape[-1] - \
            1 if t_stop_idx is None else t_stop_idx
        return self._v_r_forward(y0_batch, t_evals, params, iparams_batch,
                                 exparams_batch, ys_target_batch, t_s_i_here,
                                 t_r_i_here)
    ...
    def gradient(self,
                 ...
                 time_decay=1.0,
                 t_reset_idcs=None):
        return self._gradient_adjoint_batch(y0_batch,
                                            t_evals,
                                            params,
                                            iparams_batch,
                                            exparams_batch,
    ...
    def loss(self, ys_batch, params, iparams_batch, exparams_batch,
             ys_target_batch):
        return self._v_loss(ys_batch, params, iparams_batch, exparams_batch,
                            ys_target_batch)
...
def train_adoptODE(dataset,
                   print_interval='Auto',
                   save_interval=None,
                   print_both_losses=False):
    """Function to perform training for prepared dataset_adoptODE object.
    ...
    epochs, t_stop_idx, time_decay, t_reset_idcs = kwargs_adoptODE[
        'epochs'], kwargs_adoptODE['t_stop_idx'], kwargs_adoptODE[
            'time_decay'], kwargs_adoptODE['t_reset_idcs']
    ...
    opt_init, opt_update, get_params = build_optimizer(
        kwargs_adoptODE['lr'], kwargs_adoptODE['lr_decay'], kwargs_adoptODE['optimizer'],
        kwargs_adoptODE['optimizer_kwargs'], kwargs_adoptODE['custom_scheduel'])
    opt_state = opt_init(params_train,
    ...
    opt_init_ip, opt_update_ip, get_iparams = build_optimizer(
        kwargs_adoptODE['lr_ip'], kwargs_adoptODE['lr_decay_ip'],
        kwargs_adoptODE['optimizer_ip'], kwargs_adoptODE['optimizer_ip_kwargs'],
        kwargs_adoptODE['custom_scheduel_ip'])
    opt_state_ip = opt_init_ip(iparams_train,
    ...
    opt_init_y0, opt_update_y0, get_y0 = build_optimizer(
        kwargs_adoptODE['lr_y0'], kwargs_adoptODE['lr_decay_y0'],
        kwargs_adoptODE['optimizer_y0'], kwargs_adoptODE['optimizer_y0_kwargs'],
        kwargs_adoptODE['custom_scheduel_y0'])
    opt_state_y0 = opt_init_y0(y0,
                               lower_bound=kwargs_adoptODE['lower_b_y0'],
                               upper_bound=kwargs_adoptODE['upper_b_y0'])

    def losses_for_print(opt_state,
                         opt_state_ip,
                         opt_state_y0,
                         ys_from_grad=None):
        loss_string = ''
        if (t_reset_idcs is None and t_stop_idx is None) or print_both_losses:
            if (t_reset_idcs is None
                    and t_stop_idx is None) and not ys_from_grad is None:
                forw_full = ys_from_grad
            else:
                forw_full = forward_func(
                    y0 if t_reset_idcs is None else pytree_select_time(
                        get_y0(opt_state_y0), 0), t_evals, get_params(opt_state),
                    get_iparams(opt_state_ip), exparams)
            loss = loss_func(forw_full, get_params(opt_state),
                             get_iparams(opt_state_ip), exparams, ys_target)
            ...
            if len(loss) > 5:
                ...
                loss_print = loss
            loss_string = 'Loss: ' + ('{:.1e}, ' *
                                      len(loss_print)).format(*loss_print)
        act_loss_string = ''
        if not (t_reset_idcs is None and t_stop_idx is None):
            if ys_from_grad is None:
                y0_here = get_y0(opt_state_y0)
                if t_reset_idcs is None:
                    y0_here = tree_util.tree_map(
                        lambda x: x[:, jnp.newaxis], y0_here)
                forw_act = resetted_forward_func(y0_here, t_evals,
                                                 get_params(opt_state),
                                                 get_iparams(
                                                     opt_state_ip), exparams,
                                                 ys_target, t_stop_idx, t_reset_idcs)
            else:
                forw_act = ys_from_grad
            act_loss = loss_func(forw_act, get_params(opt_state),
                                 get_iparams(opt_state_ip), exparams, ys_target)
            if len(act_loss) > 5:
            ...
            act_loss_string = 'Stopped/Resetted Loss: ' + (
                '{:.1e}, ' * len(act_loss_print)).format(*act_loss_print)
            loss = act_loss  # If actloss is computed it is returned as the relevant loss
    ...
    for step in range(epochs):
        ...
        if (not np.isnan(print_interval)) and (step % print_interval == 0
                                               or step == epochs - 1):
            print(
                'Epoch {:03d}: '.format(step), loss_string,
                'Params Err.: {:.1e}, y0 error: {:.1e}, '
                'Params Norm: {:.1e}, iParams Err.: {:.1e}, iParams Norm: {:.1e},'.
                format(pytree_norm(params_true, get_params(opt_state)),
                       pytree_norm(dataset.y0, get_y0(opt_state_y0)),
                       pytree_norm(get_params(opt_state)),
                       pytree_norm(iparams_true, get_iparams(opt_state_ip)),
                       pytree_norm(get_iparams(opt_state_ip))), act_loss_string)
        if any([
            x.any() for x in tree_util.tree_flatten(
                tree_util.tree_map(jnp.isnan, (grads, grads_ip, grads_y0)))[0]
        ]):
            raise Exception(
        ...
        if (not kwargs_adoptODE['terminal_loss'] is None
                ) and loss < kwargs_adoptODE['terminal_loss']:
            break
    dataset.params_save = params_history
    dataset.update_sol()
    dataset.final_gradients = {'params':to_np_arr(grads),
...
def _check_kwargs_adoptODE(kwargs_adoptODE, t_evals):
    ...
    if kwargs_adoptODE[
            'EOM_dt_dependent'] is True and not 'ODE_solver' in kwargs_adoptODE:
        raise Exception(
            'dt dependent EOM is only supported if a fitting ODE_solver is '
            'provided to kwargs_adoptODE via the \'ODE_solver\' keyword.')
    unrecognised_keys = [
        key for key in kwargs_adoptODE if key not in kw_adoptODE_necessary_keys +
        list(kw_adoptODE_defaults.keys()) + kw_adoptODE_optional_keys
    ]
    if len(unrecognised_keys) > 0:
        raise Exception(
            ...
            'for one of the following? {}: '.format(
                ...
                str(kw_adoptODE_necessary_keys + list(kw_adoptODE_defaults.keys()) +
                    kw_adoptODE_optional_keys)))
    if not kwargs_adoptODE['t_reset_idcs'] is None and not isinstance(
        kwargs_adoptODE['t_reset_idcs'], tuple) and not isinstance(
            kwargs_adoptODE['t_reset_idcs'], int):
        raise Exception(
            '\'t_stop_idcs\' must be a tuple of ints, no array or list!')
    if not (kwargs_adoptODE['t_reset_idcs'] is None
        ...
        raise Exception('The last index in \'t_reset_idcs\' has to be smaller'
                        'than the \'t_stop_idx\'.')
    elif (not kwargs_adoptODE['t_reset_idcs'] is None
          ) and kwargs_adoptODE['t_stop_idx'] is None and kwargs_adoptODE[
              't_reset_idcs'][-1] >= t_evals.shape[-1] - 1:
        raise Exception('The last index in \'t_reset_idcs\' has to be smaller '
...
class dataset_adoptODE:
    ...
    Attributes:
      kwargs_adoptODE:
        Dictionary of keyword settings for the adoptODE, described below.
      def_sys_func:
      ...
      N_sys:
        The number of different systems fittet simultaneously.
      kwargs_sys:
      ...
      gen_params:
        The function generating the system params,
        returning params, iparams, exparams
      gen_y0:
        Function generating initial conditions for a single system.
      add_functions:
        Possible helper functions used by def_sys_func to construct for example
        the EOM can be returned by def_sys_func in this dictionary for later use.
      y0:
        The initial conditions for the batch of the system as provided to the
        ...
        to be passed.
      y0_train:
        The y0 value actually used during training. If the learning rate for y0
        is non-zero, this is manipulated during training to improve agreement
        with data. Has the same shape as descriped for y0.
      params_train:
        The params values used and manipulated during training.
      iparams_train:
        The iparams values used and manipulated during training.
      exparams
        The provided, known exparams values used during training.
      params:
        If provided via 'true_params' used to compare the learned params. This is
        for example the case for Physdataset_adoptODEs generated via the simple
        simulation function. Otherwise this is NaNs with the same shape as
        params_train.
      iparams:
        If provided via 'true_iparams' used to compare the learned iparams.
        Otherwise this is NaNs with the same shape as iparams_train.
      final_gradients:
        After a training run, the last values of the gradients are saved in this
        attribute as a dictionary, with the keys 'params', 'iparams' and 'y0'.
      forward:
        Function solving a batch of systems forward.
      grad_ad:
        Function using the adjoint method to compute gradients for a batch of
        systems.
      backwards_check:
        Function to check solver stability during backwards solving of the
        system.
      gradient_check:
        Function to check solver stability during backwards solving of the
        gradients.
      v_loss:
        Vectorized version of the loss function, computing the loss for a
        batch of systems.
      params_save:
        A list of the parameters at different epochs saved during training.
      ys_sol:
        The solution as computed with the current learned values for parameters
    ...
    Keywords controlling adoptODE (kwargs_adoptODE):
      'epochs':
      ...
      't_stop_idx':
        ...
        update_equations and recompiling. If used, the same code as for 
    ...
    def __init__(self,
                 ...
                 iparams_train=None,
                 y0_train=None):
        """Initializing the dataset_adoptODE
        ...
        self.f, self.sng_loss, self.gen_params, self.gen_y0, self.add_functions = def_sys_func(
            **self.kwargs_sys)

        if y0_train is None:
            if kwargs_adoptODE['t_reset_idcs'] is None:
                self.y0_train = tree_util.tree_map(
                    lambda x: np.copy(x)[:, 0], self.ys)
            else:
                self.y0_train = tree_util.tree_map(
                    lambda x: np.copy(x)[:, np.array(
        ...
        if true_params is None:
            self.params = tree_util.tree_map(lambda x: x * jnp.nan,
                                             self.params_train)
        else:
            self.params = true_params
        if true_iparams is None:
            self.iparams = tree_util.tree_map(lambda x: x * jnp.nan,
                                              self.iparams_train)
        else:
        ...
        self.equations = EquationsNODE(
            self.f, self.sng_loss, **self.kwargs_adoptODE)
        self.forward, self.gradient, self.loss, self.resetted_forward = self.equations.forward, self.equations.gradient, self.equations.loss, self.equations.resetted_forward
        self.params_save = None
        self.ys_sol = None
        self.final_gradients = None

    def update_sol(self):
        """Updates the solution saved within the class with current parameters."""
        if (self.kwargs_adoptODE['t_reset_idcs'] is None) and (self.kwargs_adoptODE['t_stop_idx'] is None):
            self.ys_sol = tree_util.tree_map(
                np.array,
                self.forward(self.y0_train, self.t_evals, self.params_train,
                             self.iparams_train, self.exparams))
        else:
            t_reset_idcs_here = (
                0,) if self.kwargs_adoptODE['t_reset_idcs'] is None else self.kwargs_adoptODE['t_reset_idcs']
            self.ys_sol = tree_util.tree_map(
                np.array,
                self.resetted_forward(self.y0_train, self.t_evals, self.params_train,
                                      self.iparams_train, self.exparams, self.ys,
                                      self.kwargs_adoptODE['t_stop_idx'],
                                      t_reset_idcs_here))

    def update_equations(self):
        """Updates and recompiles the equations."""
        self.kwargs_adoptODE = _check_kwargs_adoptODE(
            self.kwargs_adoptODE, self.t_evals)
        self.f, self.sng_loss, self.gen_params, self.gen_y0, self.add_Functions = self.def_sys_func(
            **self.kwargs_sys)
        self.equations = EquationsNODE(
            self.f, self.sng_loss, **self.kwargs_adoptODE)
        self.forward, self.gradient, self.loss, self.resetted_forward = self.equations.forward, self.equations.gradient, self.equations.loss, self.equations.resetted_forward
    ...
    def backwards_check(self,
        ...
        y0 = y0 if self.kwargs_adoptODE[
            't_reset_idcs'] is None else pytree_select_system(y0, 0)
        ...
        ys_forward = self.equations._forward(
            y0, times, params, iparams, exparams)
        a = tree_util.tree_map(jnp.zeros_like, y0)
        s0 = (tree_util.tree_map(lambda x: x[-1], ys_forward), a,
              tree_util.tree_map(lambda x: 0 * x, params),
              tree_util.tree_map(lambda x: 0 * x, iparams))
        t_evals_rev = times[-1] - jnp.flip(times)
        sol = self.equations._odeint_back(self.equations._aug_dyn,
                                          s0,
                                          t_evals_rev,
                                          params,
                                          iparams,
    ...
    def gradient_check(self,
        ...
        ys_forward = self.equations._forward(y0, t_evals, params, iparams,
                                             exparams)
        a = self.equations._dloss_dy(ys_forward, t_indices[-1], params, iparams,
                                     exparams, ys_target)
        s0 = (tree_util.tree_map(lambda x: x[t_indices[-1]], ys_forward), a,
              tree_util.tree_map(lambda x: 0 * x, params),
              tree_util.tree_map(lambda x: 0 * x, iparams))
    ...
    def nan_locator(self):
        ...
        dloss_dp = self.equations._dloss_dp(ys_forw_single, self.params_train,
                                            iparams_single, exparams_single,
                                            ys_data_single)
        dloss_dip = self.equations._dloss_dip(ys_forw_single, self.params_train,
                                              iparams_single, exparams_single,
                                              ys_data_single)
        dloss_dy = self.equations._dloss_dy(ys_forw_single,
                                            jnp.arange(len(self.t_evals)),
                                            self.params_train, iparams_single,
                                            exparams_single, ys_data_single)

        def any_nan_tree(x): return jnp.isnan(
            flatten_util.ravel_pytree(x)[0]).any()
        results = {
            'Data': ys_data,
            'Forward': ys_forw,
            'dloss_dp': dloss_dp,
...
def simple_simulation(def_sys_func,
                      t_evals,
                      kwargs_sys,
                      kwargs_adoptODE,
                      rel_noise=0.0,
    ...
    if not 'N_sys' in kwargs_sys:
        raise Exception('For simulation a key \'N_sys\' passing the number of '
    ...
    if y0 is None:
        ...
        ly0 = tree_util.tree_map(
            lambda x: np.expand_dims(x, axis=0), ly0_list[0])
        for ly0_here in ly0_list[1:]:
            ly0 = tree_util.tree_map(
                lambda x, y: np.concatenate(
                    (x, np.expand_dims(y, axis=0))), ly0,
                ly0_here)
    else:
        ly0 = tree_util.tree_map(np.array, y0)
    lys = tree_util.tree_map(np.array,
                             forward(ly0, t_evals, lparams, liparams, lexparams))
    if not kwargs_adoptODE['t_reset_idcs'] is None:
        ly0 = tree_util.tree_map(
            lambda x: np.copy(x)[:, np.array(kwargs_adoptODE['t_reset_idcs'])], lys)
    lys = tree_util.tree_map(
        lambda x: x + (rel_noise * np.mean(np.std(x, axis=1)) + abs_noise) * np.
        random.normal(size=x.size).reshape(x.shape), lys)

    return dataset_adoptODE(def_sys_func,
                            lys,
                            t_evals,
                            kwargs_sys,
                            kwargs_adoptODE,
