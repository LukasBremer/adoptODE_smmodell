...
register_pytree_node(OptimizerState, lambda xs: (
    (xs.packed_state, ),
    (xs.tree_def, xs.subtree_defs)), lambda data, xs: OptimizerState(
        xs[0], data[0], data[1]))  # type: ignore[index]
...
def optimizer(
    opt_maker: Callable[..., Tuple[Callable[[Params], State],
...
def build_cosine_restart(initial_learning_rate,
                         first_decay_steps,
                         alpha=0.0,
                         t_mul=2.0,
                         m_mul=1.0):
    ...
    def cosine_restart(step):
        ...
        cosine_decayed = 0.5 * m_fac * \
            (1.0 + np.cos(np.pi * completed_fraction))
